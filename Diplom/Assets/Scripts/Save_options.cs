...
public class Save_options : MonoBehaviour
{
    ...
    private void Profiles_load()
    {
        ...
        if (options != null)
        {
            foreach (Engine_options_class item in options)
                Item_add(item.profile_name);
            Profile_clicked(true, 0); // выбрать первый профиль как основной
            items_list[0].GetComponent<Toggle>().isOn = true;
        }
        else
    ...
    {
        Destroy(items_list[current_profile_index]);
        ...
        {
            Toggle item_toggle = items_list[i].GetComponent<Toggle>();
            item_toggle.onValueChanged.RemoveAllListeners();
            int index = i;
            item_toggle.onValueChanged.AddListener((val) => Profile_clicked(val, index));
        }
        current_profile_index = -1;
        Data_update(current_profile_index);
    }
    ...
    private void Profile_clicked(bool state, int index)
    {
        if (state)
        {
            for (int i = 0; i < items_list.Count; i++)
                if (i != index)
                    items_list[i].GetComponent<Toggle>().isOn = false;
            current_profile_index = index;
            Data_update(current_profile_index);
        }

        // нельзя отменить выбор выбранного профиля
        int active_index = items_list.FindIndex(x => x.GetComponent<Toggle>().isOn == true);
        if (active_index == -1)
            items_list[index].GetComponent<Toggle>().isOn = true;
    }
    ...
    private void Data_update(int index)
    {
        ...
        {
            profile_current.text = items_list[index].transform.Find("Label").GetComponent<Text>().text;
